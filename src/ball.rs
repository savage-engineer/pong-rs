...
impl Ball {
    ...
    pub fn kick_off(&mut self) {
        ...
        self.speed = (INIT_SPEED * multipliers.0, INIT_SPEED * multipliers.1 + 2.0);
    }

    /// Performs some sort of dodgy raycast (not raycast) thing to the next collision 
    pub fn raycast(&mut self) {
        ...
        if point.0 < self.radius {
            self.projected_x = self.radius;
        } else if point.0 > self.arena_dimensions.0 as i32 - self.radius {
            self.projected_x = self.arena_dimensions.0 as i32 - self.radius;
        } else {
    ...
    pub fn get_direction(&self) -> f64 {
        (self.speed.1 / self.speed.0).atan()
    }
}
